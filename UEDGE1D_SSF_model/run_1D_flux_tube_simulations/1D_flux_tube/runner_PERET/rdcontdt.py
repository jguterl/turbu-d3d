...
#.. zml add option to quit a running session
  #.. if one wants to quit a running case with rdcontdt.py, do:
  #.. >> touch .quit in the current run directory. It searches if a file named '.quit' exists.
import os
import sys
import numpy as np
...
exec(open('/fusion/projects/boundary/peretm/1D_flux_tube/runner_PERET/rdinitdt.py').read())

no = 0;yes = 1
echo = no

#.. zml
quit = os.path.exists('./.quit')
if quit:
    print ('.quit exist, please remove it and do again')
    #sys.exit('.quit exist, please remove it and do again')
    #quit()
    #exit()
    raise SystemExit
...
isteon=bbb.isteon
if (i_stor==0):
   ni_stor = np.zeros((bbb.n_stor,nx+1+1,ny+1+1,nisp),"d")	# set time storage arrays
   up_stor = np.zeros((bbb.n_stor,nx+1+1,ny+1+1,nisp),"d")
   te_stor = np.zeros((bbb.n_stor,nx+1+1,ny+1+1),"d")
   ti_stor = np.zeros((bbb.n_stor,nx+1+1,ny+1+1),"d")
   ng_stor = np.zeros((bbb.n_stor,nx+1+1,ny+1+1,ngsp),"d")
   phi_stor = np.zeros((bbb.n_stor,nx+1+1,ny+1+1),"d")
   tim_stor = np.zeros((bbb.n_stor),"d")
   dtreal_stor = np.zeros((bbb.n_stor),"d")
   nfe_stor = np.zeros((bbb.n_stor),"l")
...
fnrm_old = np.sqrt(sum((bbb.yldot[0:neq]*bbb.sfscal[0:neq])**2))
...
for ii1 in range( 1, bbb.ii1max+1):
   ...
   if (ii1>1  or  bbb.initjac==1):	# first time calc Jac if initjac=1
      ...
      if (bbb.iterm == 1):
         ...
         bbb.ylodt = bbb.yl
         bbb.pandf1 (-1, -1, 0, bbb.neq, 1., bbb.yl, bbb.yldot)
         fnrm_old = np.sqrt(sum((bbb.yldot[0:neq-1]*bbb.sfscal[0:neq-1])**2))
         if (bbb.dt_tot>=0.9999999*bbb.t_stop  or  fnrm_old<bbb.ftol_min):
            ...
            print ('*****************************************************')
            break
      if os.path.exists('./.quit'):   #.. zml quit if .quit file exists
         print ('jump from the inner loop because .quit')
         #sys.exit('quit because .quit is touched')
         #quit()
         #exit()
         #raise SystemExit
         quit = True
...
##       Store variables if a storage time has been crossed
            if (bbb.dt_tot >= dt_stor*i_stor and i_stor<=bbb.n_stor):
               i_stor1 = i_stor-1
               ni_stor[i_stor1,:,:,:] = bbb.ni
               up_stor[i_stor1,:,:,:] = bbb.up
               te_stor[i_stor1,:,:] = bbb.te
               ti_stor[i_stor1,:,:] = bbb.ti
               ng_stor[i_stor1,:,:,:] = bbb.ng
               phi_stor[i_stor1,:,:] = bbb.phi
   ...
   ##          End of storage section
      if os.path.exists('./.quit'):   #.. zml quit if .quit file exists
         ...
         break
   if quit:   #.. zml
      print ('jump from the outer loop because .quit')
      break
   ...
   if (bbb.iterm != 1):	#print bad eqn, cut dtreal by 3, set irev flag
      ...
      bbb.iterm = 1
if quit: print ('quit rdcontdt because .quit')  #.. zml
echo = yes
